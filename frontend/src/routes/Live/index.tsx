      ...
      <div class="p-6 h-full flex flex-col">
        ...
        <div class="flex items-center justify-between mb-4">
          <div>
            ...
            <p class="text-sm text-stone-400">Exercise: {exerciseName} — Rep 5 of 10</p>
          </div>
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-stone-500">03:24</span>
          </div>
        </div>
        ...
        <div class="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-4 min-h-0">
          ...
          <div class="lg:col-span-2 flex flex-col gap-4">
            <div class="flex-1 bg-stone-900 border border-stone-800 rounded-lg overflow-hidden relative min-h-100">
              {cameraOn ? (
                <div class="absolute inset-0 flex items-center justify-center bg-stone-900">
                  <div class="text-center">
                    ...
                    <p class="text-stone-500 text-sm">Camera feed active</p>
                    <p class="text-stone-600 text-xs mt-1">Your camera preview would appear here</p>
                  </div>
                  {/* Agent overlay feedback */}
                  <div class="absolute top-4 left-4 bg-stone-950/80 backdrop-blur-sm border border-stone-700 rounded-lg px-4 py-2">
                    <p class="text-amber-400 text-sm font-medium">✓ Good form — keep your back straight</p>
                  </div>
                  {/* Score overlay */}
                  <div class="absolute top-4 right-4 bg-stone-950/80 backdrop-blur-sm border border-stone-700 rounded-lg px-4 py-3 text-center">
                    <p class="text-xs text-stone-400">Current Score</p>
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                  <div class="text-center">
                    ...
                    <p class="text-stone-600 text-xs mt-1">Enable your camera to begin</p>
                  </div>
                </div>
              )}
            </div>
              ...
              <div class="flex items-center gap-2">
                <button
                  ...
                  class={`flex items-center gap-2 px-4 py-2.5 text-sm font-light tracking-wide transition-all ${
                    cameraOn ? 'bg-amber-500/10 text-amber-400 border border-amber-500/20' : 'bg-stone-800 text-stone-400 border border-stone-700'
                  }`}
                >
              ...
              <div class="flex-1 flex items-center gap-1 lg:ml-4 flex-wrap">
                {mockCheckpoints.map((cp, i) => (
                  <div class="flex items-center gap-1">
                    ...
                    {i < mockCheckpoints.length - 1 && <div class={`w-4 h-px ${cp.completed ? 'bg-emerald-500/30' : 'bg-stone-700'}`} />}
                  </div>
                ))}
              </div>
            </div>
