    ...
    <DashboardLayout>
      <div class="p-8">
        <div class="mb-8">
          ...
          <p class="text-gray-400 text-sm mt-1">Manage your profile, agent preferences, and view activity logs.</p>
        </div>

        {/* Tabs */}
        <div class="flex gap-1 mb-8 bg-gray-900/50 border border-gray-800 rounded-xl p-1 w-fit">
          {tabs.map(tab => (
            <button
              onClick={() => setActiveTab(tab.id)}
              class={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${
                activeTab === tab.id ? 'bg-cyan-500 text-gray-950' : 'text-gray-400 hover:text-gray-200'
              }`}
            >
              {tab.label}
            </button>
    ...
    <div class="max-w-2xl space-y-6">
      <div class="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
        ...
        <div class="space-y-5">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                value="user123"
                class="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-gray-100 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-colors"
              />
            </div>
            <div>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-300 mb-1.5">Camera Quality</label>
            <select class="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-gray-100 focus:outline-none focus:border-cyan-500 transition-colors">
              <option>480p</option>
      ...
      <div class="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
        <h3 class="text-lg font-semibold text-gray-200 mb-6">Preferences</h3>
        <div class="space-y-4">
      ...
      <button class="px-6 py-2.5 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-gray-950 font-medium transition-colors">Save Changes</button>
    </div>
  );
}
...
function AgentSettings() {
  return (
    <div class="max-w-2xl space-y-6">
      <div class="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
        <h3 class="text-lg font-semibold text-gray-200 mb-6">Agent Behavior</h3>

        <div class="space-y-5">
          <div>
            <label class="block text-sm font-medium text-gray-300 mb-1.5">Feedback Frequency</label>
            <select class="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-gray-100 focus:outline-none focus:border-cyan-500 transition-colors">
            ...
            <p class="text-xs text-gray-500 mt-1.5">How often the agent provides technique feedback.</p>
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-300 mb-1.5">Coaching Style</label>
            <select class="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-gray-100 focus:outline-none focus:border-cyan-500 transition-colors">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-300 mb-1.5">Voice</label>
            <select class="w-full px-4 py-2.5 rounded-lg bg-gray-800 border border-gray-700 text-gray-100 focus:outline-none focus:border-cyan-500 transition-colors">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-300 mb-3">Difficulty Adjustment</label>
            ...
            <p class="text-xs text-gray-500 mt-1.5">Auto adjusts based on your technique scores.</p>
          </div>
        </div>
      </div>

      <div class="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
        <h3 class="text-lg font-semibold text-gray-200 mb-6">Agent Features</h3>
        <div class="space-y-4">
      ...
      <button class="px-6 py-2.5 rounded-lg bg-cyan-500 hover:bg-cyan-400 text-gray-950 font-medium transition-colors">Save Agent Settings</button>
    </div>
  );
}
    ...
    <div class="space-y-6">
      <div class="bg-gray-900/50 border border-gray-800 rounded-xl overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
          <h3 class="text-lg font-semibold text-gray-200">Activity Logs</h3>
          <span class="text-xs text-gray-500">{mockLogs.length} entries</span>
        </div>
        <div class="divide-y divide-gray-800/50">
          {mockLogs.map(log => (
            <div class="px-6 py-3 flex items-start gap-4 hover:bg-gray-800/20 transition-colors">
              <div class={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${log.level === 'warn' ? 'bg-amber-500' : 'bg-gray-600'}`} />
              <div class="flex-1 min-w-0">
                <p class="text-sm text-gray-300">{log.event}</p>
                <div class="flex items-center gap-3 mt-1">
                  <span class="text-xs text-gray-500">{log.timestamp}</span>
                  <span class="text-xs text-gray-600">·</span>
                  <span class="text-xs text-gray-500">{log.exercise}</span>
                </div>
              </div>
            </div>
          ))}
...
function ToggleRow({ label, description, defaultOn = false }: { label: string; description: string; defaultOn?: boolean }) {
  ...
  return (
    <div class="flex items-center justify-between">
      ...
      <button onClick={() => setOn(!on)} class={`relative w-11 h-6 rounded-full transition-colors ${on ? 'bg-cyan-500' : 'bg-gray-700'}`}>
